...
function parseArgs(argv) {
  const args = { contract: null, skill: 'runtime-policy-auditor' };
...
function validateRuntimePolicyInputContract(contractPath, expectedSkill = 'runtime-policy-auditor') {
  const issues = [];
  ...
  if (!fs.existsSync(absoluteContractPath)) {
    return {
      ...
      issueCount: 1,
      issues: [`Missing local runtime-policy contract: ${absoluteContractPath}`]
    };
  }
  ...
  if (contract) {
    ...
    for (const field of ['runtimePolicyDocs', 'runtimeConfigFiles', 'controlSurfaceFiles', 'killSwitchFiles', 'approvalBoundaryFiles', 'postureOrModeDefinitions', 'healthOrStatusSurfaceFiles', 'ignorePaths']) {
      assertStringArray(issues, contract[field], field);
    }
    assertOptionalStringArray(issues, contract.optionalEnvReferenceFiles, 'optionalEnvReferenceFiles');
    ...
    for (const field of ['runtimePolicyDocs', 'runtimeConfigFiles', 'controlSurfaceFiles', 'killSwitchFiles', 'approvalBoundaryFiles', 'postureOrModeDefinitions', 'healthOrStatusSurfaceFiles', 'optionalEnvReferenceFiles']) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const result = validateRuntimePolicyInputContract(args.contract, args.skill);
  console.log(JSON.stringify(result, null, 2));
  process.exit(result.ok ? 0 : 1);
}

export { validateRuntimePolicyInputContract };
