...
| `runtime/service/service-readiness.mjs` | validator | n/a | validator-backed | `npm run runtime:service` and runtime service tests | service-mode readiness gate only; blocks service start without explicit flag and keeps HTTP/MCP/remote transport gated behind local auth/permission readiness |
| `runtime/auth/auth-context.mjs` | validator | n/a | validator-backed | `npm run runtime:service -- --preflight --identity local-user` and runtime service tests | resolves bounded local runtime identities from controlled sources only; no login system or user store |
| `runtime/auth/claim-binding.mjs` | validator | n/a | validator-backed | used by `runtime/service/service-preflight.mjs` | binds `identity + scope + action + target`; default blocks missing identity, implicit trust, and unbound actions |
| `runtime/service/service-preflight.mjs` | validator | n/a | validator-backed | `npm run runtime:service -- --preflight --identity local-user` | local auth/PBC preflight only; can pass checks while still returning `serviceStartAllowed: false` |
| `runtime/service/service-actions.mjs` | validator | n/a | validator-backed | `npm run runtime:service -- --simulate-action run --identity local-user` and runtime service tests | defines local-only `run`, `status`, `replay`, and `cancel` action contracts with explicit identity-bound claims and action-level permission coverage |
| `runtime/service/service-action-receipts.mjs` | runnable-helper | n/a | runnable | invoked by `npm run runtime:dry-run` | writes local `service-actions.json` receipts with identity, explicit claim, claim binding result, and disabled transport/service-start fields |
| `runtime/service/service-request-envelope.mjs` | helper-script | n/a | helper-only | used by `runtime/service/service-request-validation.mjs` | builds local request envelopes with controlled identity, endpoint, action, claim, and disabled service-start fields |
| `runtime/service/service-request-validation.mjs` | validator | n/a | validator-backed | `npm run runtime:service -- --validate-request POST /runtime/service/run --identity local-user` | validates local request envelope to endpoint/action/identity-bound claim mapping; no listener, HTTP/MCP, remote transport, or daemon |
| `runtime/cli/runtime-service.mjs` | runnable-tool | n/a | stub | `npm run runtime:service` | fail-closed service-mode boundary CLI; no HTTP listener, MCP server, remote transport, background service, or daemon |
...
| `artifacts/runtime-runs/<runId>/` | generated artifact surface | n/a | implemented | created by `npm run runtime:dry-run`; validated by `npm run runtime:validate` | ignored local runtime-implemented evidence containing manifest, events, permissions, memory, handoff, resource, trigger, service action, and validation receipt artifacts |
